...
public class ImpKruskal {
	
	graph g;
	
	public ImpKruskal(Edge e[], Vertex v[]) {
	...
	public static void main(String[] args) {
		...
		ImpKruskal obj = new ImpKruskal(e , vert);		
	...
	public void Kruskal(graph g)
	{
		...
		{
			System.out.println("("+g.edg[i].v1.item+","+g.edg[i].v2.item+","+g.edg[i].weight+")");
		}
		for(int i = 0; i< g.edg.length; i++)
		{
			...
			if(!v1.item.equals(v2.item))
			{
				Union(v1,v2);
				totalcost+= g.edg[i].weight;
				System.out.println("("+v1.item+","+v2.item+")");
			}
		}
